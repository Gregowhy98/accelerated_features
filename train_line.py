...
def parse_arguments():
    ...
    parser.add_argument('--synthetic_root_path', type=str, default='/home/datasets/coco_20k',
                        help='Path to the synthetic dataset root directory.')
    parser.add_argument('--wireframe_root_path', type=str, default='/home/datasets/wireframe',
                        help='Path to the synthetic dataset root directory.')
    ...
    parser.add_argument('--training_type', type=str, default='xfeat_default',
                        choices=['xfeat_default', 'xfeat_synthetic', 'xfeat_megadepth', 'xfeat_wireframe'],
...
from modules.dataset.wireframe.wireframedataset import WireframeDataset
...
class Trainer():
    ...
    def __init__(self, megadepth_root_path, 
        ...
        ##################### WIREFRAME INIT ##########################
        # TODO:
        if model_name in ('xfeat_wireframe'):
            
            TRAIN_LINE_MAT_PATH = f"{wireframe_root_path}/line_mat"
            TRAIN_IMG_PATH = f"{wireframe_root_path}/V1.1"
            
            data = WireframeDataset(TRAIN_IMG_PATH, TRAIN_LINE_MAT_PATH, use='train')
            self.data_loader = DataLoader(data, 
    ...
    def train(self):
        ...
        with tqdm.tqdm(total=self.steps) as pbar:
            for i in range(self.steps):     # 单次迭代
                if not self.dry_run:
                    if self.data_iter is not None:
                        try:
